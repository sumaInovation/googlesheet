...
app.post('/api/auth/google', async (req, res) => {
  ...
  try {
    ...
    const role = users[email] || 'guest';

    // Generate a JWT for the user
    const jwtToken = jwt.sign({ email, role }, 'your_jwt_secret', { expiresIn: '1h' });

    res.json({ message: 'Login successful', role, token: jwtToken });
  } catch (error) {
